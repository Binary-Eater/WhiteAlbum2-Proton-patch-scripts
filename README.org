...
~lpVtbl~. +If I move this dll out of the game's loading path, the game will directly use DXVK's ~d3d9.dll~ and the issue is mitigated since the ~lpVtbl~ is no longer corrupted.+

UPDATE: Thanks to a user reporting an [[https://github.com/Binary-Eater/WhiteAlbum2-Proton-patch-scripts/issues/1][issue]], I learned that ~d3d9.dll~ comes
from the [[https://github.com/TodokanaiTL/subtitles][translation project]]. From debugging the DLL further, I came to realize
the problem comes from this intercept dll always destroying COM objects on any
~Release~ call instead of when the reference count of the object drops to zero.
Microsoft explicitly [[https://learn.microsoft.com/en-us/windows/win32/api/unknwn/nf-unknwn-iunknown-release][documents]] that a COM object should only clean itself up
when the reference count has dropped to zero. Previously, Direct3DWrapper and
Direct3DDeviceWrapper would try to free themselves irrespective of the refcount
of the original instances that they wrap. This is problematic especially with
DXVK where the instances have a refcount greater than one. Actually, even when
debugging wine's OpenGL-based D3D9 implementation, the refcount was also greater
than zero but it seems the memory was not zero-ed, so the lpVtbl for the COM
object was still in a reliable state for the subsequent Release calls. I have
published a [[https://github.com/TodokanaiTL/subtitles/pull/1][pull request]] to resolve this upstream in the translation project.
The dll is actually important since it patches in English subtitles into the
movie scenes. I did not notice this myself since I can understand Japanese, but
I am illiterate in the language. Here is an image of dll working with DXVK while
presenting additional debug information.

#+ATTR_HTML: :width 60%
[[./images/dll-working-with-dxvk.png]]
...
Using ~PROTON_USE_WINED3D=1~ is another possible workaround, but I personally
prefer to use DXVK whenever possible. To patch the White Album 2 installation
for resolving this issue, you can simply run something along the lines of the
following. This script will copy over the fixed dll without the additional debug
information being presented on the game, that was shown previously in this
README. However, that dll is provided under ~./dlls/debug/d3d9.dll~ if needed.

#+BEGIN_SRC shell
  ~/Documents/wa2-patch-scripts
  ❯ ./wa2-proton-dxvk-patch.sh ~/.local/share/wineprefixes/wa2/drive_c/Leaf/WHITE\ ALBUM2
  Found WHITE ALBUM 2 d3d9.dll from the translation project
  https://github.com/TodokanaiTL/subtitles

  Replacing the d3d9.dll with one including a bugfix for DXVK...
